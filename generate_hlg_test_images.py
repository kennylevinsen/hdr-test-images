#!/usr/bin/env python3
"""
Generate HLG HEIF test images for validating the BT.2100 OOTF fix in GTK4.

Produces four files encoding the same intended display appearance:
  test_reference.jpg    — sRGB JPEG ground truth
  test_srgb.heif        — HEIF with CICP 1/13/0/1 (sRGB)
  test_hlg.heif         — HEIF with CICP 9/18/0/1 (HLG BT.2020, SDR γ=0.78)
  test_hlg_1000nit.heif — HEIF with CICP 9/18/0/1 (HLG BT.2020, 1000-nit γ=1.2)

With a correct HLG OOTF implementation, all files should look identical.
...
def hlg_inverse_ootf(display_linear, system_gamma=0.78, luminance_scale=1.0):
    """
    ...
    Yd = np.sum(display * REC2020_LUMA, axis=-1, keepdims=True)
...
def encode_hlg_heif(chart_linear, output_path, system_gamma, luminance_scale):
    """
    Linear sRGB → BT.2020 → inverse OOTF → HLG OETF → 16-bit → ffmpeg PNG → heif-enc.
    CICP 9/18/0/1 (BT.2020 primaries, HLG transfer, RGB matrix, full range).
    """
